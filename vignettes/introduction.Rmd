...
## Use full data
result1 <- gevrFit(data = x, method = "mle", locvars = covs, locform = ~ Trend1 + Trend2*Trend3,
scalevars = covs, scaleform = ~ Trend1)

## Only use r = 1
result2 <- gevrFit(data = x[, 1], method = "mle", locvars = covs, locform = ~ Trend1 + Trend2*Trend3,
scalevars = covs, scaleform = ~ Trend1)
...
result3 <- gevrFit(data = x, method = "mle", locvars = covs, locform = ~ Trend1,
scalevars = covs, scaleform = ~ Trend1)

result4 <- gevrFit(data = x, method = "mle", locvars = covs, locform = ~ Trend1,
...
LRT <- as.numeric(2 * (logLik(result3) - logLik(result4)))
...
Next, we fit another model, with 
