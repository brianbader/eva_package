...
gpdSeqTests <- function(data, thresholds = NA, nextremes = NA, method = c("cvm", "ad", "pbscore", "multscore", "imasym", "impb"),
  ...
  for(i in 1:num) {
    ...
    result[i, 1] <- i
    if(method == "cvm")
      fit <- gpdCvm(x, nsim, allowParallel, numCores)
    if(method == "ad")
      fit <- gpdAd(x, nsim, allowParallel, numCores)
    if(method == "pbscore")
      fit <- gpdPbScore(x, nsim, information = information, allowParallel, numCores)
    if(method == "multscore")
      fit <- gpdMultScore(x, nsim, information = information)
    if(method == "imasym")
      fit <- gpdImAsym(x, nsim)
    if(method == "impb")
      fit <- gpdImPb(x, inner, outer, allowParallel, numCores)
